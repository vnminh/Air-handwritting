...
\begin{table}[t]
\centering
\caption{Positional-encoding and augmentation ablations at seed 42. All rows use S+D+F, gated fusion, $K=4$, and otherwise default settings; each block varies only the named factor.}
\label{tab:components}
\begin{tabular}{llrrrr}
\toprule
Factor & Setting & Val. CER (\%) & CER (\%) & WER (\%) & Exact (\%)\\
\midrule
PE & None & \textbf{2.16} & \textbf{1.74} & \textbf{5.52} & \textbf{94.82}\\
PE & Absolute & 4.74 & 4.12 & 10.30 & 91.83\\
PE & Relative & 2.18 & 2.40 & 6.93 & 93.94\\
\midrule
Augmentation & Disabled & \textbf{1.51} & \textbf{1.57} & \textbf{4.97} & \textbf{94.64}\\
Augmentation & Enabled & 2.18 & 2.40 & 6.93 & 93.94\\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[t]
\centering
\caption{Fourier-scale, preprocessing, and resampling-length ablations at seed 42 ($K=4$ except the Fourier-scale block itself; S+D+F, gated fusion, relative PE otherwise default). ``Raw'' disables smoothing, spline interpolation, and normalization.}
\label{tab:secondary}
\resizebox{\linewidth}{!}{%
...
The selected $K=2$ model produces 207 character-level edit operations over 11,721 reference characters (101 substitutions, 81 deletions, 25 insertions; CER 1.77\%) and 181 word-level errors over 3,261 reference words (WER 5.55\%); 2,157 of 2,276 test trajectories are transcribed exactly (94.77\%). The relative-position Conformer reduces CER from 11.79\% (Transformer) to 1.85\%, confirmed over three seeds ($1.94\pm0.11\%$ vs.\ $14.76\pm2.77\%$); recurrent baselines remain far weaker. The component ablations (Tables~\ref{tab:components}--\ref{tab:secondary}) show no single factor dominates in isolation -- no positional encoding and disabled augmentation both beat the default configuration alone -- so the $K=2$ checkpoint's accuracy should be attributed to the joint configuration rather than any one component; length $L=192$ and spline-only preprocessing each individually improve on the $K=4$ default, suggesting headroom the selected checkpoint does not yet capture.

Figure~\ref{fig:qualitative-bao-gio} shows exact recognition of \emph{bao giờ} (156 raw points), preserving the space and diacritic. Figure~\ref{fig:qualitative-long} shows the harder 415-point phrase \emph{ngọn núi cao vời vợi}, also decoded exactly, with mean spatial/dynamic/Fourier gate weights 0.162/0.454/0.384 -- the dynamic branch dominates while the others stay active, consistent with the corpus-wide gate analysis above. Figure~\ref{fig:qualitative-bi} shows a failure case: \emph{bí} decoded as \emph{bú}, a base-vowel substitution rather than a segmentation error (length and tone mark are retained). This is plausible because the short vowel-defining movement occupies only a small part of the trajectory and can be weakened by smoothing/resampling or confused with jitter. Samples with two to seven non-space characters have CER 0.31--0.48\%, but samples with eight or more rise to 4.59\% and contribute 178 of the 207 character errors -- long trajectories remain the principal failure mode even though all inputs are compressed to the same 128-point representation.

\begin{figure}[t]
\centering
\begin{minipage}{0.32\linewidth}\centering\includegraphics[width=\linewidth]{figures/qualitative_bao_gio.png}\end{minipage}\hfill
\begin{minipage}{0.32\linewidth}\centering\includegraphics[width=\linewidth]{figures/qualitative_ngon_nui_cao_voi_voi.png}\end{minipage}\hfill
\begin{minipage}{0.32\linewidth}\centering\includegraphics[width=\linewidth]{figures/qualitative_bi_error.png}\end{minipage}
\caption{Left: correct \emph{bao giờ}. Center: correct long phrase \emph{ngọn núi cao vời vợi}. Right: failure case, \emph{bí} recognized as \emph{bú}.}
